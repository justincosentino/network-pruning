...
def init_flags():
    ...
    tf.flags.DEFINE_integer("batch_size", default=128, help="The batch size")
    tf.flags.DEFINE_integer(
        "num_valid", default=10000, help="The size of the validation dataset"
    )
    tf.flags.DEFINE_integer("epochs", 20, "Number of training epochs to perform.")
    tf.flags.DEFINE_float("learning_rate", 0.001, "The optimizer's learning rate.")
    tf.flags.DEFINE_float("l1_reg", 0, "l1 regularization lambda")
    tf.flags.DEFINE_float("l2_reg", 0, "l2 regularization lambda")
    tf.flags.DEFINE_list(
        "k_vals",
        [0.0, 0.25, 0.50, 0.60, 0.70, 0.80, 0.90, 0.95, 0.97, 0.99, 0.995, 0.999],
...
def train(experiment_dir):
    ...
    (x_train, y_train, x_valid, y_valid, x_test, y_test) = load_data(FLAGS.dataset)(
...
def prune_experiments(experiment_dir):
    ...
    (_, _, _, _, x_test, y_test) = load_data(FLAGS.dataset)(num_valid=FLAGS.num_valid)
    ...
    for k in FLAGS.k_vals:
        ...
        weight_loss, weight_acc = weight_pruned_model.evaluate(x=x_test, y=y_test)
        ...
        graph.plot_units_l2(
            unit_pruned_model, experiment_dir, k, "pruned_units_l2_k={}.png".format(k)
        )
...
def main(_):
    ...
    experiment_dir = os.path.join(FLAGS.output_dir, FLAGS.experiment_id)
